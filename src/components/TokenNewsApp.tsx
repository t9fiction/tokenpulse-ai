...
import { Search, TrendingUp, AlertCircle, Clock, ExternalLink, Star, Filter, Bell, Target, ArrowUp, ArrowDown, DollarSign, RefreshCw, Wifi, WifiOff } from 'lucide-react';
...
const TokenNewsApp: React.FC = () => {
  ...
  const fetchCryptoData = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const formattedTokens: Token[] = data.map((token) => {
        ...
        // Calculate support and resistance levels based on 24h data
        const support = low24h * 0.98; // Slightly below 24h low
        const resistance = high24h * 1.02; // Slightly above 24h high
        
        // Generate mock price history (in a real app, you'd fetch historical data)
        const priceHistory = Array.from({ length: 5 }, (_, i) => 
  ...
  const fetchCryptoNews = useCallback(async (): Promise<void> => {
    try {
      // Using NewsAPI - you'll need to replace 'YOUR_API_KEY' with actual API key
      // For demo purposes, using a free alternative or mock data
  ...
  // Generate trading suggestions based on real price analysis
  ...
  const getSelectedTokenData = (): Token | undefined => {
    return tokens.find(token => 
      token.symbol === tokenMapping[selectedToken as keyof typeof tokenMapping]?.symbol
    );
  };
  ...
  }, [fetchCryptoData, fetchCryptoNews, isLive]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button className="px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-medium rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all">
                Connect Wallet
              </button>
            </div>
          </div>
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {tokenData && (
                        <span className={`text-sm font-medium ${
                          tokenData.changePercent >= 0 ? 'text-green-600' : 'text-red-600'
                        }`}>
                          {tokenData.change}
                        </span>
                      )}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-gray-50 rounded-lg p-3">
                      ...
                      <div className={`text-xl font-bold ${
                        selectedTokenData.changePercent >= 0 ? 'text-green-600' : 'text-red-600'
                      }`}>
            ...
            {(['all', 'positive', 'negative', 'trending'] as const).map((filter) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium transition-all capitalize ${
                  newsFilter === filter
                    ? `${
                        filter === 'positive' ? 'bg-green-600' : 
                        filter === 'negative' ? 'bg-red-600' : 
                        filter === 'trending' ? 'bg-blue-600' : 'bg-purple-600'
          ...
          {filteredNews.map((news) => (
            ...
            >
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3">
                  <span className={`px-3 py-1 rounded-full text-sm font-medium capitalize ${getSentimentColor(news.sentiment)}`}>
                    {news.sentiment}
                  </span>
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${getRelevanceColor(news.relevance)}`}>
